  ...
  useEffect(() => {
    imageGroups.forEach((group) => {
      group.images.forEach((img) => {
        if (img.imageRef) {
          ...
          img.imageRef.addEventListener("mouseenter", () => {
            gsap.to(img.imageRef, { scale: 1.1, duration: 0.3 });
          });

          img.imageRef.addEventListener("mouseleave", () => {
    ...
    return () => {
      imageGroups.forEach((group) => {
        group.images.forEach((img) => {
          if (img.imageRef) {
            img.imageRef.removeEventListener("mouseenter", null);
            img.imageRef.removeEventListener("mouseleave", null);
          }
        });
      });
    };
  ...
  const fetchImages = async () => {
    ...
    try {
      const response = await fetch(
        "https://meowseum-backend-production.up.railway.app/api/submissions",
        {
          method: "GET",
          credentials: "include",
        }
